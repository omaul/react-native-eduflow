...
function createBranch(p: p5, x: number, y: number, angle: number, depth: number): Branch {
  ...
  return {
    ...
    growProgress: 0,
    growSpeed: p.random(0.004, 0.012),
    children: [],
    hasFlower: depth >= 2 && p.random() > 0.3,
    flowerSize: p.random(4, 10),
    flowerHue: p.random(300, 380) % 360,
    petalCount: p.floor(p.random(4, 8)),
    depth,
  };
}
...
function growBranch(p: p5, branch: Branch, maxDepth: number) {
  if (branch.growProgress < 1)
    branch.growProgress = p.min(1, branch.growProgress + branch.growSpeed);
  if (branch.growProgress >= 1 && branch.children.length === 0 && branch.depth < maxDepth) {
    ...
    for (let i = 0; i < num; i++) {
      ...
      branch.children.push(
        createBranch(p, endX, endY, branch.angle + p.random(-0.7, 0.7), branch.depth + 1),
      );
    }
  }
...
function drawBranch(p: p5, branch: Branch) {
  ...
  if (branch.hasFlower && prog >= 1) {
    ...
    for (let i = 0; i < branch.petalCount; i++) {
      ...
      p.ellipse(
        p.cos(a) * branch.flowerSize * 0.55,
        p.sin(a) * branch.flowerSize * 0.55,
        branch.flowerSize,
        branch.flowerSize * 0.6,
      );
    }
    p.fill(45, 75, 70, 220);
...
function initSeeds(p: p5, w: number, h: number): Seed[] {
  ...
  for (let i = 0; i < p.floor(p.random(10, 20)); i++) {
    seeds.push({
      ...
      driftY: p.random(-0.35, -0.08),
      phase: p.random(p.TWO_PI),
      lineLen: p.random(8, 16),
    });
  }
  return seeds;
}
...
function drawSeed(p: p5, s: Seed) {
  ...
  const tx = s.x + p.sin(sw) * s.lineLen,
    ty = s.y - p.cos(sw) * s.lineLen;
  p.stroke(80, 25, 65, 90);
  p.strokeWeight(0.6);
  p.line(s.x, s.y, tx, ty);
  p.stroke(60, 15, 80, 60);
  p.strokeWeight(0.3);
  for (let i = 0; i < 5; i++) {
    const a = (p.TWO_PI / 5) * i + p.frameCount * 0.002;
    p.line(tx, ty, tx + p.cos(a) * s.size * 2, ty + p.sin(a) * s.size * 2);
  }
  p.noStroke();
  p.fill(60, 20, 90, 100);
...
function initFerns(p: p5, w: number, h: number): Fern[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    ferns.push({
      ...
      side,
      maxSegments: p.floor(p.random(12, 22)),
      growProgress: 0,
      growSpeed: p.random(0.003, 0.008),
...
function initBorderFlowers(p: p5, w: number, h: number): BorderFlower[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    flowers.push({
      ...
      bloomProgress: 0,
      bloomSpeed: p.random(0.005, 0.015),
    });
  }
  return flowers;
}
...
const _ELEMENT_NAMES = ['branches', 'vines', 'seeds', 'petals', 'ferns', 'flowers'] as const;
type ElementName = (typeof _ELEMENT_NAMES)[number];
...
export function createPlantsSketch(seed: string) {
  return function plantsSketch(p: p5) {
    ...
    p.draw = () => {
      ...
      for (const f of state.ferns) {
        updateFern(f);
        drawFern(p, f);
      }
      for (const f of state.borderFlowers) {
        updateBorderFlower(f);
        drawBorderFlower(p, f);
      }
